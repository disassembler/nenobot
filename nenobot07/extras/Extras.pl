...
sub Extras {
	...
	if ($found == 1) {
	    if ($msgType !~ /public/) {
		&msg($who, "\002[\002$k\002]\002 Forgotten.");
	    } else {
		&say("\002[\002$k\002]\002 Forgotten.");
	    }
	    $l = $who; $l =~ s/^=//;
	    $updateCount++;
	    return 'NOREPLY';
    ...
    if ($message =~ /^\s*die (.*)\s*$/) {
	if ($msgtype =~ /public/i) {
		&msg(&channel(), "\002[\002die - error\002]\002 You can't do that in public.");
	}
	if ($1 eq $param{'diepass'}) {
		&killed();
	} else {
		&msg($who, "\002[\002die - error\002]\002 Invalid password.");
	}
    }

    if ($message =~ /^fortune/) { # minddog++ for idea.
	$channel = &channel();
	$fortune = `fortune`;
	$fortune =~ s/\n/ /g;
	$fortune =~ s/\t/ /g;
	&say("\002[\002Fortune\002]\002 $fortune");
	return 'NOREPLY';
    }
	...
	if ($code =~ /^[a-zA-Z][a-zA-Z0-9]{3,4}$/) {
	    ...
	    $weath = "\002[\002Weather - error\002]\002 Try a 4-letter station code (see http://www.nws.noaa.gov/tg/siteloc.shtml for locations and codes)";
	}
...
# modified to fork and generally search by oznoid
	...
	if ($seen) {
	    ...
	    if ($msgType =~ /public/) {
		&performSay("\002[\002seen - $person\002]\002 I last saw $person in $where, about $tstring ago, saying $what [$when]");	
	    } else {
		&msg("\002[\002seen - $person\002]\002 I last saw $person in $where, about $tstring ago, saying $what [$when]");	
	    }
	    return 'NOREPLY';
	}
...
&status("Karma string is currently \'$message2\'");
	    ...
	    if ($karma) {
		return "\002[\002$message2 - karma rating\002]\002 $karma";
	    } else {
		return "\002[\002$message2 - karma rating\002]\002 0";
	    }
	}
    }
