...
# check "is" tables anyway for lhs alone
	...
	if ($msgType !~ /private/ and $theMsg =~ s/^\s*<reply>\s*//i) {
	    ...
	    $shortReply = 1;
		 ...
		 $theMsg =~ /^<action>/i) {
	    ($token, $actionstr) = split(" ", $theMsg, 2); # nenolod++ (faster)
	    $actionstr =~ s/<who>/\Q$target\E/g; # The new who token
	    $actionstr =~ s/\Q$who\E/$target/g; # compatibility.
	    &action(&channel(), $actionstr); #this is public addressing.
	    $theMsg = 'NOREPLY';
	    $shortReply = 1;
	} elsif ($msgType =~ /private/ and
		$theMsg =~ /^<action>/i) {
            ...
            $actionstr =~ s/<who>/\Q$target\E/g; # The new who token
            $actionstr =~ s/\Q$who\E/$target/g; # compatibility.
	    &action($target, $actionstr); #this -is- private addressing.
	    $theMsg = 'NOREPLY';
	    $shortReply = 1;
	} else {		# not a short reply
	    if (!$infobots{$nuh} and $theVerb =~ /is/) {
		$theMsg = "\002[\002$orig_Y\002]\002 $theMsg";
	    }
	}
    }
