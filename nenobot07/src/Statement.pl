...
sub doStatement {
	...
	while ($in =~ s/(\(.*?\)|[^(++)(--)\s]+)(\+\+|--)//) {
	    ...
	    if ($msgType !~ /public/i) {
		&msg($who, "\002[\002karma - error\002]\002 karma must be done in public!");
		return "NOREPLY";
	    }

	    if (lc($term) eq lc($who)) {
		$linetoreturn = "\002[\002karma - error\002]\002 please don't karma yourself";
		&msg(&channel(), $linetoreturn);
		return "NOREPLY";
	    }
	...
	# Whoops!  Better make sure that we're adding karma to something existant.
		...
		$linetoreturn = "\002[\002karma - success\002]\002 karma for $term modified.";
	}
		&msg(&channel(), $linetoreturn) if $linetoreturn;
		undef $linetoreturn;
